...
export default function FeedView({ categories, cards, card, currentIndex, activeCategory, hasVoted, onCategoryChange, onPrevious, onNext, onShuffle, onVote, onAddComment }) {
  ...
  return <section className="editorial-feed relative flex h-full w-full min-h-0 flex-col items-center">
    ...
    <article onPointerDown={startCardGesture} onPointerMove={moveCardGesture} onPointerUp={finishCardGesture} onPointerCancel={() => { gestureStart.current = null; setIsDraggingMedia(false); setDragOffset(0); }} onWheel={moveCardByWheel} className={`relative z-10 h-full min-h-0 w-full touch-none overflow-hidden rounded-xl border border-surface-container-high/60 bg-[#fbfaf7] shadow-2xl ${feedMotion}`}>
      ...
      <div className="absolute left-0 top-0 z-30 flex w-full items-start justify-between px-4 pt-6"><div className="flex items-center gap-1.5 rounded-full border border-white/20 bg-black/35 px-2 py-0.5 shadow-lg backdrop-blur-sm"><span className="h-1.5 w-1.5 animate-pulse rounded-full" style={{ backgroundColor: theme.color, boxShadow: `0 0 8px ${theme.color}` }} /><span className="font-mono text-[8px] font-bold tracking-wide text-white">LIVE STREAM</span><span className="font-mono text-[8px] text-white/75">{card.timestamp}</span></div></div>
      <div className="absolute left-0 top-0 z-30 flex w-full items-start justify-between px-4 pt-11"><div className="flex items-center gap-1 rounded-full border border-white/20 bg-black/35 px-2 py-0.5 shadow-lg backdrop-blur-sm"><span className="material-symbols-outlined text-[14px]" style={{ color: theme.color }}>{theme.icon}</span><span className="font-mono text-[11px] font-semibold uppercase tracking-wider text-white">{theme.feedLabel ?? card.category}</span></div><div className="flex items-center gap-1.5 rounded-full border border-white/20 bg-black/35 py-0.5 pl-1 pr-2.5 shadow-lg backdrop-blur-sm"><span className="flex h-4.5 w-4.5 items-center justify-center rounded-full bg-primary-container/80"><span className="material-symbols-outlined text-[12px] text-white">person</span></span><span className="font-headline text-[11px] font-semibold tracking-wide text-white">@{card.author}</span></div></div>
...
function Result({ yesPercent, noPercent, total, color, onNext }) { const noColor = '#B42318'; return <div className="mb-2.5 rounded-xl border border-surface-container-high bg-surface-container-low/95 p-2.5 backdrop-blur"><div className="mb-1 flex items-center justify-between font-mono text-xs font-bold"><span className="flex items-center gap-1" style={{ color }}><span className="material-symbols-outlined text-[14px]">thumb_up</span> YES {yesPercent}%</span><span className="text-[#4b5563]">총 {total.toLocaleString()}명 참여</span><span className="flex items-center gap-1" style={{ color: noColor }}>NO {noPercent}% <span className="material-symbols-outlined text-[14px]">thumb_down</span></span></div><div className="flex h-2 w-full overflow-hidden rounded-full border border-[#101828] bg-slate-800"><span className="result-bar--yes" style={{ width: `${yesPercent}%`, backgroundColor: color }} /><span className="result-bar--no" style={{ width: `${noPercent}%`, backgroundColor: noColor }} /></div><div className="mt-1.5 flex items-center justify-between"><span className="font-mono text-[10px] text-[#4b5563]">신뢰도 지수: <strong className="text-[#101828]">96.2 / 100</strong></span><button type="button" onClick={onNext} className="flex items-center text-[11px] font-bold" style={{ color }}>다음 사진 <span className="material-symbols-outlined text-[13px]">arrow_downward</span></button></div></div>; }
...
function CommentPanel({ card, media, comments, draft, onDraftChange, onClose, onSubmit }) {
  ...
  return <section className="comment-panel" role="dialog" aria-modal="true" aria-label="게시물 댓글 상세">
    <div className="comment-panel__dialog">
      ...
      <div className="comment-panel__media"><CardMedia card={card} media={mediaItems[mediaIndex]} className="h-full w-full object-contain" muted />{mediaItems.length > 1 && <><button type="button" onClick={() => setMediaIndex((index) => (index - 1 + mediaItems.length) % mediaItems.length)} aria-label="이전 사진" className="comment-panel__media-nav comment-panel__media-nav--left"><span className="material-symbols-outlined">chevron_left</span></button><button type="button" onClick={() => setMediaIndex((index) => (index + 1) % mediaItems.length)} aria-label="다음 사진" className="comment-panel__media-nav comment-panel__media-nav--right"><span className="material-symbols-outlined">chevron_right</span></button><span className="comment-panel__media-count">{mediaIndex + 1} / {mediaItems.length}</span></>}</div>
