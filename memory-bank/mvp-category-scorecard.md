...
| **비교군 3 — 여행** | 3 | 3 | 3 | 4 | 2 | 3 | 4 | 4 | **26** | 높은 공유성 대비 Result·Boost 전환 강도를 비교 |

## MVP 운용 결정

- 프로토타입은 현재의 **7개 카테고리**를 유지한다. 이는 동일 UI·평가 구조가 카테고리별로 재사용되는지 검증하기 위함이다.
- Closed Beta의 우선 모집·노출 순서는 **`PERCEIVED_AGE` → `오늘의 룩` → `SNS 프로필` → `데이트`**로 확정한다.
- `운동`, `출근`, `여행`은 위 순서의 비교군으로 유지하며, Weekly Valid Votes·Upload→Result 완료율·Share율이 낮으면 Paid/Boost 노출 대상에서 제외한다.
...
- [x] 7개 후보의 질문·평가 방식·초기 Cash Loop 가설 기록
